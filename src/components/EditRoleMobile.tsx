...
import { Bike, User, UserCog, Phone, ArrowRight, Loader2, Check } from 'lucide-react'
import axios from 'axios'

const EditRoleMobile = () => {
    const router = useRouter()
    const [roles] = useState([
        { id: "user", label: "Customer", desc: "Browse items, place orders, and track deliveries", icon: User },
        { id: "deliveryBoy", label: "Delivery Partner", desc: "Deliver orders, manage deliveries, and earn money", icon: Bike },
        { id: "admin", label: "Administrator", desc: "Manage system, users, roles, and settings", icon: UserCog }
    ])
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            const res = await axios.post("/api/auth/user/edit-role-mobile", {
                role: selectedRole, mobile: mobile.trim() 
            })
            if (!res.data.success) {
                                ...
                                return (
                                    ...
                                    >
                                        ...
                                        <div className={`p-3 rounded-xl mb-3 ${
                                            isSelected ? "bg-green-100 text-green-700" : "bg-slate-50 text-slate-400 group-hover:text-slate-500"
                                        }`}>
                                            <Icon className="w-6 h-6" />
                                        </div>
                                        <div>
                                            <h3 className="font-bold text-sm tracking-wide mb-1">{role.label}</h3>
